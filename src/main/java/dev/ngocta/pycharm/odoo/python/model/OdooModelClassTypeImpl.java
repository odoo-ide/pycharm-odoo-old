...
public class OdooModelClassTypeImpl extends UserDataHolderBase implements PyCollectionType {
    private OdooModelClass myClass;
    private OdooRecordSetType myRecordSetType;

    private OdooModelClassTypeImpl(@NotNull OdooModelClass source, @NotNull OdooRecordSetType recordSetType) {
        myClass = source;
        myRecordSetType = recordSetType;
    }

    public static OdooModelClassTypeImpl create(@NotNull String model, @NotNull OdooRecordSetType recordSetType, @NotNull Project project) {
        OdooModelClass cls = OdooModelClass.create(model, project);
        return create(cls, recordSetType);
    }

    public static OdooModelClassTypeImpl create(@NotNull OdooModelClass source, @NotNull OdooRecordSetType recordSetType) {
        ConcurrentMap<OdooRecordSetType, OdooModelClassTypeImpl> cache = CachedValuesManager.getCachedValue(source, () -> {
            return CachedValueProvider.Result.create(new ConcurrentHashMap<>(), ModificationTracker.NEVER_CHANGED);
        });
        OdooModelClassTypeImpl classType = cache.get(recordSetType);
        if (classType == null) {
            classType = new OdooModelClassTypeImpl(source, recordSetType);
    ...
    @Nullable
    public static OdooModelClassTypeImpl create(@NotNull PyClass source, @NotNull OdooRecordSetType recordSetType) {
    ...
    public OdooModelClassTypeImpl getOneRecordVariant() {
        return OdooModelClassTypeImpl.create(myClass, OdooRecordSetType.ONE);
    }

    public OdooModelClassTypeImpl getMultiRecordVariant() {
        return OdooModelClassTypeImpl.create(myClass, OdooRecordSetType.MULTI);
    }
    ...
    public List<PyClassLikeType> getSuperClassTypes(@NotNull TypeEvalContext context) {
        ...
        for (PyClass cls : myClass.getSuperClasses(context)) {
            if (cls instanceof OdooModelClass) {
                result.add(OdooModelClassTypeImpl.create(cls, myRecordSetType));
            } else {
    ...
    public PyClassLikeType toInstance() {
        return myRecordSetType != null ? this : OdooModelClassTypeImpl.create(myClass, OdooRecordSetType.MODEL);
    }

    @NotNull
    @Override
    public PyClassLikeType toClass() {
        return myRecordSetType == null ? this : OdooModelClassTypeImpl.create(myClass, OdooRecordSetType.NONE);
    }
    ...
    private LookupElement getCompletionLine(@NotNull PsiNamedElement element, @NotNull TypeEvalContext context) {
        ...
        if (name != null) {
            ...
            if (element instanceof PyTargetExpression) {
                OdooFieldInfo info = OdooFieldInfo.readFromClassAttribute((PyTargetExpression) element, context);
                if (info != null) {
